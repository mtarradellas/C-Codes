...
by commas.
*/
int storesFiles(censusADT c, char *pathCountry, char * pathProvince, char * pathDepartment, int * emptyFile);

//Auxiliary function used in storesFiles.
int storeCountry(censusADT c, FILE * fileCountry);

//Auxiliary function used in storesFiles.
int storeProvincesAndDeptartment(censusADT c, FILE * fileProvince, FILE * fileDepartment, int * emptyFile);
...
int main(void){
	...
	//Empty file is 0 when the file with data might be empty.
	int status, x, lineError=1, memError=1, storeError, emptyFile=1;
	...
	*/
	...
	storeError = storesFiles(c, PATH_COUNTRY, PATH_PROVINCE, PATH_DEPARTMENT, &emptyFile);
...
int storesFiles(censusADT c, char *pathCountry, char * pathProvince, char * pathDepartment, int * emptyFile){
	...
	flag2 = storeProvincesAndDeptartment(c, fileProvince, fileDepartment, emptyFile);
